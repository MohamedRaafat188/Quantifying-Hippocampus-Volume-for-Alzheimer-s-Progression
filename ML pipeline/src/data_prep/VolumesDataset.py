...
class VolumesDataset(Dataset):
    ...
    def __init__(self, data):
        ...
        self.volumes = [i for i, _ in enumerate(data)]
    ...
    def __getitem__(self, idx):
        ...
        image = self.data[volume]['image'][:, :, :]
        image = image[None, :, :, :]
        image = torch.from_numpy(image).type(torch.FloatTensor)  # type: ignore
        sample['image'] = image

        seg = self.data[volume]['seg'][:, :, :]
        seg = seg[:, :, :]
